...
The prior specification treated several improvements as “transfer”, including absolute→relative changes. This v3.3 specification separates three different phenomena:
...
The programme is designed around a **20-session target envelope**, not a fixed phase timetable. Session bands are nominal planning expectations. The app should introduce key wrapper swaps when the current learning curve is stable enough to make the swap meaningful.

```text
20 sessions = consumer programme envelope
wrapper swaps = learning-curve-triggered events
transfer metrics = computed at carrier-swap boundaries
```
...
### 4.2 Twenty-session target envelope with learning-curve-triggered wrapper swaps

The commercial programme is designed as a **20-session target envelope**, not a fixed phase schedule.

The app must not hard-code wrapper transitions by session number. Instead, it should begin in `P1_ARROW_ABS` and advance through the ordered phase sequence using learning-curve evidence.
...
Nominal phase bands are useful for planning, UX expectation-setting and product analytics, but they are not implementation constraints. The actual phase transition is controlled by the learning-curve controller.

#### Nominal pathway

| Nominal band | Phase | Primary cell(s) | Purpose | Actual transition rule |
|---:|---|---|---|---|
| ~Sessions 1–5 | `P1_ARROW_ABS` | `arrow_abs` | establish first-carrier absolute baseline and pre-swap update geometry | advance when `arrow_abs` reaches flattening / stable baseline |
| ~Sessions 6–8 | `P2_FLOW_ABS` | `flow_abs` | measure recovery when the same absolute relation appears in optic flow | starts after P1 flattening; advance when `flow_abs` has enough recovery evidence |
| ~Sessions 9–12 | `P3_ARROW_REL` | `arrow_rel` | establish the within-arrow absolute→relative learning signature | advance when `arrow_rel` reaches flattening / stable relational baseline |
| ~Sessions 13–15 | `P4_FLOW_REL` | `flow_rel` | validate relational abstraction in flow and test relational carrier transfer | starts after P3 flattening; advance when `flow_rel` has enough recovery evidence |
| ~Sessions 16–18 | `P5_MIXED` | all four cells | test flexible switching across carrier × frame cells | start when the four-cell lattice has enough evidence; continue until mixed stability evidence is adequate |
| ~Sessions 19–20 | `P6_DELAYED` | all four cells | delayed / spaced return check | run when spacing and prior phase evidence permit; otherwise mark delayed transfer as still calibrating |

Compact expectation:

```text
target envelope ≈ 20 sessions
nominal planning shape ≈ 5 + 3 + 4 + 3 + 3 + 2
actual swaps = learning-curve-triggered
```

This means the nominal session bands can be displayed to the user as a typical journey, but the engine should treat them as **soft planning bands**, not fixed switches.
...
```text
session count suggests the likely phase,
but learning-curve evidence controls the actual wrapper swap.
```

The app should only introduce a key transfer swap when the active cell has reached adequate local stability.
...
These are the primary carrier-swap transfer events and must not be triggered merely because the user has reached a scheduled session number.

#### If the user is not ready by the nominal band

If the active cell has not reached the learning-curve criterion by the end of its nominal band, the app should continue training the current cell while preserving the overall 20-session progress framing.

User-facing language:

```text
Current phase: Direction Foundation
New challenges appear when your current learning curve is stable.
```

or:

```text
Your next challenge is coming up. We are keeping you here a little longer so the baseline is clearer.
```

Do not present this as failure.

Backend labels may include:

```text
phase_extended_for_learning_curve
needs_more_evidence
timing_limited
still_calibrating
```

#### If the user progresses faster than expected

If the active cell reaches flattening earlier than the nominal band, the app may introduce the next wrapper earlier, provided minimum data requirements and timing-quality checks are met.

Do not advance after a single good block.

Minimum readiness should include:

```text
minimum valid trials
stable or flattening capacity curve
accuracy in or above training band
stable lapse rate
acceptable timing quality
```

#### Twenty-session completion rule

The 20-session programme remains the standard commercial completion target, but the exact phase reached by session 20 may differ across users.
...
Relation Recovery
Mixed Flexibility
Return Strength, if available
```

If a user has not reached all planned phases by session 20, mark later transfer components as:

```text
calibrating
not enough evidence yet
continue in self-guided practice
```

rather than forcing invalid transfer scores.

After session 20, self-guided gameplay can continue collecting data and may complete any transfer components that were still calibrating.

#### Research-mode extension rule

For validation cohorts only, the programme may extend beyond the 20-session commercial envelope when:
...
OR a planned carrier-swap boundary has not yet been reached because the prior cell has not flattened
```
...
### 4.3 Nominal within-session allocation

These allocation rules apply to the **current active phase**, whenever that phase occurs. They should not be tied to hard session numbers.
...
### 4.4 Construct scheduling inside the 20-session target envelope

The 20-session target envelope defines the intended **carrier × frame progression**. It can be applied to `ACC` alone or to both `ACC` and `BSE`, but the trial budget must be explicit.
...
If both `ACC` and `BSE` are scored in the same 20-session target envelope, use one of these trial-budget policies:
...
| Sequential programmes | 20 sessions ACC, then 20 sessions BSE | Cleanest research route for BSE transfer, but not the same commercial envelope. |
...
### 5.3 Learning-curve phase controller

The app should implement a phase controller rather than a fixed session router.

The controller should evaluate the active construct × cell after each session and decide whether the next wrapper can be introduced.

Pseudocode:

```typescript
function chooseNextPhase(userState) {
  const phase = userState.currentPhase

  if (phase === "P1_ARROW_ABS" && hasFlattened("ACC", "arrow_abs")) {
    return "P2_FLOW_ABS"
  }

  if (phase === "P2_FLOW_ABS" && hasRecoveryEvidence("ACC", "flow_abs")) {
    return "P3_ARROW_REL"
  }

  if (phase === "P3_ARROW_REL" && hasFlattened("ACC", "arrow_rel")) {
    return "P4_FLOW_REL"
  }

  if (phase === "P4_FLOW_REL" && hasRecoveryEvidence("ACC", "flow_rel")) {
    return "P5_MIXED"
  }

  if (phase === "P5_MIXED" && hasMixedStabilityEvidence()) {
    return "P6_DELAYED"
  }
...
For `BSE`, use one of two policies:

| Policy | Rule |
|---|---|
| ACC-led progression | ACC controls wrapper transitions; BSE follows the same phase and is marked lower-confidence if lagging. |
| Construct-parity progression | Both ACC and BSE must meet minimum readiness for key transfer swaps. This is cleaner scientifically but may slow the consumer pathway. |

Recommended v1:

```text
ACC-led progression
+ BSE confidence labels
+ explicit BSE lag flags
```

This preserves a usable consumer programme while still collecting BSE transfer evidence.

Do not allow a key carrier-swap transfer event unless the previous cell has a valid pre-swap baseline.

Required transition-event checks:

```text
minimum valid trials in source cell
minimum valid trajectory windows in source cell
acceptable timing quality
stable or flattening capacity curve
no unresolved global fatigue / timing-limited flag
```

If readiness is not met, continue the current phase and show the user a simple “still building baseline” message.
...
ADD COLUMN IF NOT EXISTS phase_transition_mode text DEFAULT 'learning_curve'
  CHECK (phase_transition_mode IN ('learning_curve','manual_research_override')),
ADD COLUMN IF NOT EXISTS nominal_session_band text,
...
8. run the learning-curve phase controller
9. update phase state only if readiness / recovery criteria are met
10. if a transition boundary exists, update transfer metrics
```
...
- [ ] Implement the learning-curve phase controller; do not hard-code wrapper swaps by session number.
- [ ] Treat session bands as nominal UX/planning bands only.
...
- [ ] Present the 20-session journey as a typical pathway, not a fixed phase timetable.
- [ ] Use “new challenges appear when your current learning curve is stable” rather than fixed unlock dates.
