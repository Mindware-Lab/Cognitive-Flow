...
import {
  ...
  type AuthUser,
  type IqCoachPurchaseProductCode,
...
type SuiteAccessStatus = "checking" | "active" | "missing" | "error";
...
let suiteAccessStatus: SuiteAccessStatus = isIqCoachCommerceEnabled ? "checking" : "active";
let checkoutBusy = false;
let accessMessage = checkoutState === "complete"
  ? "Payment received. Check your email for the six-digit IQ Mindware sign-in code, then use it here to activate your access."
  : checkoutState === "cancelled"
    ? "Checkout was cancelled. No payment was taken."
    : checkoutState === "access"
      ? "Finish signing in with the email address used at checkout to unlock your access."
      : "";
...
function setView(next: View): void {
  if (paidAccessRequired() && suiteAccessStatus !== "active" && next !== "access" && next !== "auth") {
    next = "access";
  }
  clearTaskTiming();
  if (next !== "task") {
  ...
  return shell(`
    <section class="ccc-real-life-checkin-card">
      ...
      <div class="ccc-real-life-option-grid">
        ...
        <button class="ccc-real-life-option" data-action="bridge-probe-response" data-probe-response="unsure">Unsure</button>
      </div>
  ...
  return shell(hasJourney ? `
    ...
    <section class="ccc-hero">
      <div class="ccc-hero-copy">
        ...
        <p class="ccc-app-title">Cognitive Control Coach</p>
...
function renderAuth(): string {
  ...
  return shell(`<section class="ccc-auth-screen">
    <span class="ccc-kicker">Sign in</span><h1>${paidAccessRequired() ? "Use the email address from checkout." : "Continue with your IQ Mindware account."}</h1>${form}
    ${accountMessage ? `<p class="ccc-account-message">${escapeHtml(accountMessage)}</p>` : ""}
    ${paidAccessRequired() ? `<button class="ccc-button ccc-button-quiet" data-action="return-access">Back to access options</button>` : ""}
...
function commitRealLifePractice(replacePending = false): void {
  if (!journey) return;
  const practice = finalRealLifePractice();
  if (!practice) return;
...
async function refreshSuiteAccess(user: AuthUser | null = authUser): Promise<boolean> {
  if (!paidAccessRequired()) {
    suiteAccessStatus = "active";
    return true;
  }
  if (!user) {
    suiteAccessStatus = "missing";
    view = "access";
    render();
    return false;
  }
  ...
  suiteAccessStatus = "checking";
  if (view === "access") render();
  try {
    const entitlement = await resolveIqCoachAccess("cognitive_control_coach");
    if (!entitlement) {
      suiteAccessStatus = "missing";
      accessMessage = `No Cognitive Control Coach entitlement was found for ${user.email || "this account"}. Sign in with the checkout email or choose a route that includes the Coach.`;
      view = "access";
      render();
      return false;
    }
    ...
    suiteAccessStatus = "error";
    accessMessage = error instanceof Error ? error.message : "Your Cognitive Control Coach access could not be checked.";
    view = "access";
    render();
    return false;
  }
...
async function startProductCheckout(productCode: IqCoachPurchaseProductCode): Promise<void> {
  if (checkoutBusy) return;
  checkoutBusy = true;
  accessMessage = "Opening Stripe’s secure checkout…";
  render();
  try {
    const checkoutUrl = await createIqCoachCheckoutSession(productCode);
    window.location.assign(checkoutUrl);
  } catch (error) {
    checkoutBusy = false;
    accessMessage = error instanceof Error ? error.message : "Stripe Checkout could not be opened.";
    render();
  }
}
...
appRoot.addEventListener("click", (event) => {
  ...
  if (action === "start-product-checkout") {
    const productCode = button.dataset.productCode;
    if (productCode === "cognitive_control_coach" || productCode === "complete_cognitive_route") {
      void startProductCheckout(productCode);
    }
  } else if (action === "access-sign-in") {
    accountMessage = "";
    setView("auth");
  } else if (action === "return-access") {
    accountMessage = "";
    setView("access");
  } else if (action === "retry-suite-access") {
    void refreshSuiteAccess();
  } else if (action === "choose-tester-exercise") {
    ...
    void signOutUser().then(() => {
      ...
      if (paidAccessRequired()) {
        suiteAccessStatus = "missing";
        accessMessage = "Signed out. Use the email address from checkout to restore access.";
        view = "access";
      }
      populationScores = {};
      comparisonMode = dataMode === "cloud_benchmark" ? "population" : "personal";
...
if (isSupabaseConfigured) {
  void currentAuthUser().then(async (user) => {
    authUser = user;
    if (paidAccessRequired()) {
      const allowed = await refreshSuiteAccess(user);
  ...
  onAuthChange((user) => {
    ...
    if (paidAccessRequired()) {
      ...
      void refreshSuiteAccess(user).then(async (allowed) => {
        if (!allowed) return;
        if (dataMode !== "local") {
          cloudStatus = "Signed in. Progress sync is on.";
          await hydrateCloudProgress(user);
