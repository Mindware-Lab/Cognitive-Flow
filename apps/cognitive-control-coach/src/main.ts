...
import { DEFAULT_GUIDED_NO_INPUT_TRIAL_LIMIT, blockCompletionGuard, guidedSessionCompletionGuard, nextNoInputTrialCount, shouldExitForNoInput } from "./sessionCompletion";
...
type ProofMobileTab = "attention" | "other";
...
const GENERATOR_VERSION = "cognitive-control-coach-generator-v0.1";
const ADAPTIVE_VERSION = "cognitive-control-coach-staircase-v0.1";
const SCORING_VERSION = "cognitive-control-coach-scoring-v0.1";
...
function protocolAssignmentSeed(): string {
  return `cognitive-control-coach:${PROTOCOL_ASSIGNMENT_VERSION}:${currentBrowserDeviceId}`;
}
...
async function hydrateScratchBaselines(): Promise<void> {
  ...
  try {
    const baselines = await fetchCoachScratchBaselines({
...
function standardScoreMap(rows: StandardizedScoreRow[]): Record<string, StandardizedScoreSummary> {
  ...
  for (const row of rows) {
    const existing = byMetric[row.metric_key];
    if (existing) {
      const existingTime = Date.parse(existing.recordedAt || "");
      const rowTime = Date.parse(row.recorded_at || "");
      const existingSession = Number(existing.sessionNumber || 0);
      const rowSession = Number(row.session_number || 0);
      const rowIsOlder =
        (Number.isFinite(existingTime) || Number.isFinite(rowTime))
          ? (Number.isFinite(rowTime) ? rowTime : 0) <= (Number.isFinite(existingTime) ? existingTime : 0)
          : rowSession <= existingSession;
      if (rowIsOlder) continue;
    }
...
async function hydrateStandardizedScores(): Promise<void> {
  ...
  try {
    state.standardizedScores = standardScoreMap(await loadStandardizedScores("cognitive_control_coach"));
    render();
  } catch (error) {
    ...
    <main class="app-shell">
      <header class="app-brand-bar">
        ...
        <div class="app-header-brand">
          ${preAuthGate ? "" : `<img src="${assetPath("cognitive-control-coach-wordmark.svg")}" alt="Cognitive Control Coach" />`}
        </div>
...
function appTabs(active: "today" | "session" | "progress" | "coaching"): string {
  return `
    <nav class="tabs app-tabs" aria-label="Cognitive Control Coach sections">
...
function brainNetworkDiagram(): string {
  return `
    <img
      ...
      src="${assetPath("cognitive-control-network.png")}"
...
function dataRemovalCard(cloudActive: boolean): string {
  return `<button class="data-mode-card data-removal-card is-reset" data-action="delete-attention-data">
    ...
    <p>${cloudActive ? "Delete your cloud Cognitive Control Coach data and reset this browser." : "Reset this browser's local Cognitive Control Coach progress."}</p>
...
async function exportCurrentData(): Promise<void> {
  try {
    if (cloudSyncActive()) {
      const exported = await exportCoachData();
      downloadJson(`cognitive-control-coach-cloud-export-${todayIso()}.json`, exported);
      markSync("synced", "Cloud data export created.");
    } else {
      downloadJson(`cognitive-control-coach-local-export-${todayIso()}.json`, {
...
async function deleteCurrentData(): Promise<void> {
  const target = cloudSyncActive() ? "cloud Cognitive Control Coach data and local browser progress" : "local Cognitive Control Coach progress in this browser";
  ...
  try {
    ...
    state.viewHistory = [];
    markSync(state.cloudSyncMode === "cloud" ? "synced" : "local", "Cognitive Control Coach data deleted.");
  ...
  return shell(`
    <section class="splash-card">
      ...
      <section class="splash-brand">
        ...
        <img src="${assetPath("cognitive-control-coach-wordmark.svg")}" alt="Cognitive Control Coach" class="splash-wordmark" />
  ...
  return shell(`
    <section class="panel session-overview-panel">
      <div class="session-overview-copy">
        <p class="ui-eyebrow">Session ${currentGuidedSessionNumber()} - ${escapeHtml(phaseStatusCopy(phase, state.progress.phaseStatus))}</p>
        <h1>${intro.title}</h1>
...
function renderTask(): string {
  ...
  const endAction = state.sessionMode === "protocol" ? "exit-guided-session" : "end-block";
  const endLabel = state.sessionMode === "protocol" ? "Exit" : "End";
  const endKey = state.sessionMode === "protocol" ? "X" : "E";
  ...
  return shell(`
    <section class="task-main">
      ...
      <div class="task-controls">
        ...
        <button class="task-skip-button" data-action="${endAction}"><span>${endKey}</span> ${endLabel}</button>
      </div>
    </section>
  `, { task: true });
}
  ...
  return `
    <section class="interblock-progress-card">
      <div class="interblock-progress-copy">
        ...
        ${feedback.contextText ? `<p class="interblock-context-note">${escapeHtml(feedback.contextText)}</p>` : ""}
      </div>
...
function gTrackPublicStandardScore(publicScores: Record<string, unknown> | null | undefined, key: string): number | null {
  ...
  const record = value as { displayStandardScore?: unknown; standardScore?: unknown; score?: unknown };
  return finiteStandardScore(record.displayStandardScore) ?? finiteStandardScore(record.standardScore) ?? finiteStandardScore(record.score);
}
...
function gTrackScoreValue(score: GTrackHistoryRow["gTrackScore"]): number | null {
  ...
  const directScore = finiteStandardScore((score as GTrackProofScore | null | undefined)?.displayStandardScore)
    ?? finiteStandardScore((score as GTrackProofScore | null | undefined)?.standardScore)
    ?? finiteStandardScore((score as GTrackProofScore | null | undefined)?.score);
  ...
  if (score?.construct === "attention_control") {
  ...
  return `
    <section class="proof-form-card">
      <div>
        ...
        <p>Use this for your own external benchmark records. Entries stay separate from Cognitive Control Coach training scores and are not certificates, credentials, or institutional reports.</p>
      </div>
      <label>Domain
...
function renderProof(): string {
  return shell(`
    ${appTabs("progress")}
    <section class="proof-screen proof-overview-screen is-proof-tab-${state.proofMobileTab}">
      ...
      <div class="proof-mobile-tabs" role="tablist" aria-label="Proof graph group">
        <button type="button" data-action="nav-proof-tab-attention" class="${state.proofMobileTab === "attention" ? "is-active" : ""}" role="tab" aria-selected="${state.proofMobileTab === "attention" ? "true" : "false"}">Attention</button>
        <button type="button" data-action="nav-proof-tab-other" class="${state.proofMobileTab === "other" ? "is-active" : ""}" role="tab" aria-selected="${state.proofMobileTab === "other" ? "true" : "false"}">Memory + Matrix</button>
      </div>
...
function scoreDisplayForMetric(metric: ProgressScoreMetric, personalDelta: number | null, currentScore: number | null): number | null {
  if (!benchmarkScoringSelected()) return personalIndexFromDelta(personalDelta);
  return benchmarkScoreForMetric(metric) ?? currentScore;
}

function statusForMetricDisplay(metric: ProgressScoreMetric, personalDelta: number | null, currentScore: number | null): string {
  ...
  if (standardScore === null) return statusForScore(currentScore);
...
function publicScoreContextLabelForMetric(metric: ProgressScoreMetric): string {
  if (!benchmarkScoringSelected()) return "personal index";
  return benchmarkScoreForMetric(metric) === null ? "training score" : "benchmark score";
}

function latestHistoryScoreForMetric(metric: ProgressScoreMetric): number | null {
  for (const entry of [...(state.progress.scoreHistory || [])].reverse()) {
...
function snapshotScoreForMetric(metric: ProgressScoreMetric): number | null {
  const snapshot = state.progress.latestSnapshot;
  if (!snapshot) return null;
  if (metric === "cognitiveBandwidth") return snapshot.attentionControl.trainingScore;
  if (metric === "patternBinding") return snapshot.bindingFocus.trainingScore;
  if (metric === "transfer") return snapshot.transfer.score;
  if (metric === "frameBandwidth") return snapshot.transfer.relationRecovery.score;
  if (metric === "wrapperRecovery") {
    return averageScore([
      snapshot.transfer.motionRecovery.score,
      snapshot.transfer.relationRecovery.score,
    ]);
  }
  if (metric === "delayedRecovery") return snapshot.transfer.returnStrength.score;
  return null;
}

function scoreWithProgressFallback(metric: ProgressScoreMetric, evidenceScore: number | null): number | null {
  return evidenceScore ?? latestHistoryScoreForMetric(metric) ?? snapshotScoreForMetric(metric);
}
...
function progressDashboardPresentationModel(): ProgressDashboardPresentationModel {
  ...
  const signalScore = scoreWithProgressFallback("cognitiveBandwidth", metricScores.cognitiveBandwidth);
  const relationalScore = scoreWithProgressFallback("frameBandwidth", metricScores.frameBandwidth);
  const bindingScore = scoreWithProgressFallback("patternBinding", metricScores.patternBinding);
  const wrapperRecoveryScore = scoreWithProgressFallback("wrapperRecovery", averageScore([
    snapshot?.transfer.motionRecovery.score ?? null,
    snapshot?.transfer.relationRecovery.score ?? null,
  ]));
  const returnScore = scoreWithProgressFallback("delayedRecovery", snapshot?.transfer.returnStrength.score ?? null);
  ...
  return {
    ...
    transferDelta: scoreDisplayForMetric("transfer", transferRelative.delta, transferReadiness),
    ...
    skills: [
      {
        ...
        scoreDelta: scoreDisplayForMetric("cognitiveBandwidth", cognitiveRelative.delta, signalScore),
        baseline: cognitiveRelative.baseline,
        status: statusForMetricDisplay("cognitiveBandwidth", cognitiveRelative.delta, signalScore),
        statusNote: statusNoteFor(statusForMetricDisplay("cognitiveBandwidth", cognitiveRelative.delta, signalScore)),
      ...
      },
      {
        ...
        scoreDelta: scoreDisplayForMetric("frameBandwidth", frameRelative.delta, relationalScore),
        baseline: frameRelative.baseline,
        status: statusForMetricDisplay("frameBandwidth", frameRelative.delta, relationalScore),
        statusNote: statusNoteFor(statusForMetricDisplay("frameBandwidth", frameRelative.delta, relationalScore)),
      ...
      },
      {
        ...
        scoreDelta: scoreDisplayForMetric("patternBinding", bindingRelative.delta, bindingScore),
        baseline: bindingRelative.baseline,
        status: statusForMetricDisplay("patternBinding", bindingRelative.delta, bindingScore),
        statusNote: statusNoteFor(statusForMetricDisplay("patternBinding", bindingRelative.delta, bindingScore)),
      ...
      },
      {
        ...
        scoreDelta: scoreDisplayForMetric("wrapperRecovery", wrapperRelative.delta, wrapperRecoveryScore),
        baseline: wrapperRelative.baseline,
        status: statusForMetricDisplay("wrapperRecovery", wrapperRelative.delta, wrapperRecoveryScore),
        statusNote: statusNoteFor(statusForMetricDisplay("wrapperRecovery", wrapperRelative.delta, wrapperRecoveryScore)),
      ...
      },
      {
        ...
        scoreDelta: scoreDisplayForMetric("delayedRecovery", delayedRelative.delta, returnScore),
        baseline: delayedRelative.baseline,
        status: statusForMetricDisplay("delayedRecovery", delayedRelative.delta, returnScore),
        statusNote: statusNoteFor(statusForMetricDisplay("delayedRecovery", delayedRelative.delta, returnScore)),
...
function renderSessionProgressCard(model: ProgressDashboardPresentationModel): string {
  return `
    <section class="session-progress-card session-progress-chart-card">
      ...
      (skill, index) => `
        <div class="detail-skill-row public-skill-row ${dashboardToneClass(skill.tone)}" title="${escapeHtml(skill.subtitle)}">
          ...
          <span class="public-skill-score">
            ...
            <small>${skill.scoreDelta === null ? "More guided data needed" : publicScoreContextLabelForMetric(skill.metric)}</small>
          </span>
  ...
  return shell(`
    ...
    <section class="coaching-screen">
      <div class="coaching-hero">
        ...
        <p>Use a short coaching session to review your Cognitive Control Coach pattern, decide the right pace, and choose what to do next.</p>
...
function beginSession(): void {
  ...
  state.consecutiveNoInputTrials = 0;
  state.feedback = "";
  state.taskStage = "ready";
...
function exitGuidedSession(): void {
  clearActiveSession();
  go("today");
}
...
function endCurrentBlock(): void {
  ...
  const block = state.sessionPlan?.miniBlocks[state.activeBlockIndex];
  const guard = blockCompletionGuard(block, state.blockResults);
  if (!guard.completed) {
    exitGuidedSession();
    return;
  }
...
function submitCurrentGuidedBlock(results: TrialResult[]): void {
  ...
  const submission = submitCoachBlock(blockSubmissionPayload(plan, block, results)).catch((error) => {
...
function finalizeGuidedSession(input: {
  ...
  pendingBlockSubmissions = [];
  void Promise.allSettled(submissions).then(() => finalizeCoachSession({
...
function completeSession(): void {
  ...
  const completionGuard = guidedSessionCompletionGuard(state.sessionPlan, state.sessionResults);
  if (!completionGuard.completed) {
    exitGuidedSession();
    return;
  }
...
function answerTrial(response: string | null): void {
  ...
  state.consecutiveNoInputTrials = nextNoInputTrialCount(state.consecutiveNoInputTrials, response);
  ...
  if (
    state.sessionMode === "protocol" &&
    shouldExitForNoInput(state.consecutiveNoInputTrials, DEFAULT_GUIDED_NO_INPUT_TRIAL_LIMIT)
  ) {
    exitGuidedSession();
    return;
  }
...
appRoot.addEventListener("click", async (event) => {
  ...
  if (action === "send-login-link") {
  ...
  else if (action === "nav-proof-tab-attention" || action === "nav-proof-tab-other") {
    state.proofMobileTab = action === "nav-proof-tab-attention" ? "attention" : "other";
    go("proof", { replace: true });
  }
  ...
  else if (action === "nav-evidence") go("training-map");
  ...
  else if (action === "exit-guided-session") exitGuidedSession();
    ...
    if (!window.confirm("Start a new 20-30 session programme? This resets your Cognitive Control Coach training progress, but keeps separate proof records.")) return;
