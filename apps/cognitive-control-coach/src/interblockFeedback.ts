...
export function getInterblockPhaseLabel(input: {
  ...
  if (input.postSwap && (accuracyTrend === "falling" || coreTrend === "falling")) return "expected_dip";
  if (isMixedPhase(input.phase) && accuracyTrend !== "falling" && coreTrend !== "falling") return "mixed_stable";
  if (input.postSwap && (accuracyTrend === "rising" || coreTrend === "rising")) return "recovery_starting";
...
function transitionContextFor(point: BlockFeedbackPoint | null): string | null {
  if (!point?.transitionKey) return null;
  const transition = TRANSITIONS[point.transitionKey];
  if (!transition) return null;
  if (transition.type === "carrier_swap") {
    return "Format changed: a short dip is expected; recovery shows the same skill carrying into the new wrapper.";
  }
  if (transition.type === "frame_ramp") {
    return "Rule frame changed: a short dip is expected while you recover the same control skill around the centre.";
  }
  if (transition.type === "mixed_switch") {
    return "Formats are now mixed: stable performance means the rule is holding across changing wrappers.";
  }
  if (transition.type === "delayed_recheck") {
    return "Delayed re-check: this shows whether the skill returns after time away.";
  }
  return null;
}
...
export function buildInterblockFeedback(input: {
  ...
  return {
    ...
    contextText: transitionContextFor(latest),
