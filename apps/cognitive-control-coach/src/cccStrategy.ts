import { CCC_REGIMES, CCC_TRIAL_TIMING } from "./cccConfig";
...
function features(responseTimeMs: number, ratio: CccRatio): readonly [number, number, number, number] {
  const time = (Math.max(CCC_TRIAL_TIMING.minimumExposureBeforeAnswerMs, responseTimeMs)
    - CCC_TRIAL_TIMING.minimumExposureBeforeAnswerMs)
    / Math.max(1, CCC_TRIAL_TIMING.maxResponseWindowMs - CCC_TRIAL_TIMING.minimumExposureBeforeAnswerMs);
 ...
 */
function fitAccuracyModel(results: readonly CccRecordedTrial[]): AccuracyModel | null {
  const answered = results.filter((result) => result.scoring.responseClass === "answer"
    && result.scoring.responseTimeMs !== null);
  if (answered.length < MIN_MODEL_ANSWERS) return null;
  const responseTimes = answered.map((result) => Number(result.scoring.responseTimeMs));
  ...
  for (let iteration = 0; iteration < 700; iteration += 1) {
    ...
    for (const result of answered) {
      const x = features(Number(result.scoring.responseTimeMs), result.trial.ratio);
...
function probabilityCorrect(model: AccuracyModel, responseTimeMs: number, ratio: CccRatio): number {
  const x = features(responseTimeMs, ratio);
  return logistic(model.intercept * x[0] + model.time * x[1] + model.mixed * x[2] + model.close * x[3]);
}

function expectedValueAt(model: AccuracyModel, regimeId: CccRegimeId, responseTimeMs: number): number {
  ...
  return (Object.entries(regime.ratioPriors) as Array<[CccRatio, number]>).reduce((total, [ratio, weight]) => {
    const correct = probabilityCorrect(model, responseTimeMs, ratio);
    return total + weight * (correct * reward - (1 - correct) * regime.errorLoss);
  }, 0);
}

function bestEstimatedTime(model: AccuracyModel, regimeId: CccRegimeId): { timeMs: number; value: number } {
  let best = { timeMs: CCC_TRIAL_TIMING.minimumExposureBeforeAnswerMs, value: Number.NEGATIVE_INFINITY };
  for (let timeMs = CCC_TRIAL_TIMING.minimumExposureBeforeAnswerMs;
    timeMs <= CCC_TRIAL_TIMING.maxResponseWindowMs;
    timeMs += 50) {
    const value = expectedValueAt(model, regimeId, timeMs);
...
function strategyForRegime(
  ...
  const answered = results.filter((result) => result.trial.regimeId === regimeId
    ...
    && result.scoring.responseTimeMs !== null);
  const observedMedianMs = median(answered.map((result) => Number(result.scoring.responseTimeMs)));
  ...
  const best = bestEstimatedTime(model, regimeId);
  const observedValue = expectedValueAt(model, regimeId, observedMedianMs);
...
export function buildCccStrategyFeedback(
  ...
  const model = fitAccuracyModel(eligible);
  return {
    modelReady: model !== null,
    answeredCount: eligible.filter((result) => result.scoring.responseClass === "answer").length,
    regimes: regimeIds.map((regimeId) => strategyForRegime(regimeId, eligible, model)),
