...
export const wmTransferDefaults = {
  baseFlatteningMinValidTrials: 88,
  baseFlatteningMinWindows: 3,
...
function evidenceErrorStable(evidence: CellEvidence | null): boolean {
  return Boolean(
    evidence &&
      evidence.lapseRate <= 0.18 &&
      evidence.falseAlarmRate <= 0.35 &&
      evidence.lureErrorRate <= 0.35
  );
}
...
export function chooseNextTransferState(input: WapUserState): WapDecision {
  ...
  const readiness = {
    ...
    lapseStable: evidenceErrorStable(activeEvidence),
