...
async function hydrateStandardizedScores(): Promise<void> {
  ...
  try {
    state.standardizedScores = standardScoreMap(await loadStandardizedScores("wm_coach"));
    render();
  } catch (error) {
...
const STANDARD_SCORE_METRIC_KEYS: Record<ProgressScoreMetric, string> = {
  ...
  cognitiveBandwidth: "control.stable_n_level",
...
function scoreUnitLabel(): string {
  return benchmarkScoringSelected() ? "std" : "Δ";
}
...
function transferPillTone(status: string): string {
  if (status === "Strong" || status === "Transfer holding" || status === "Above benchmark") return "green";
  if (status === "Bottleneck" || status === "Below benchmark") return "red";
  if (status === "Calibrating" || status === "N-level emerging" || status === "Benchmark pending") return "blue";
  return "orange";
}
