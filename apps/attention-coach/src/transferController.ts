...
export const attentionTransferDefaults = {
...
export const TRANSFER_DEFAULTS = attentionTransferDefaults;
...
function phaseFromLegacy(phase: PhaseLabel, path: TransferPath): Pick<
  ...
  | "legacyStatus"
> {
  if (phase === "P2_FLOW_ABS" || phase === "P2_ARROW_ABS") {
    return legacyPatch(path.startWrapper, path.carrierTargetWrapper, "recovering", null, null, "clean", false, false, "legacy_active");
  }
  if (phase === "P3_ARROW_REL" || phase === "P3_FLOW_REL") {
    return legacyPatch(path.startWrapper, path.frameTargetWrapper, "recovering", null, null, "clean", false, false, "legacy_active");
  }
  if (phase === "P4_FLOW_REL" || phase === "P4_ARROW_REL") {
    return legacyPatch(path.frameTargetWrapper, path.heldOutWrapper, "recovering", null, null, "legacy_exposed", path.heldOutWrapper === "flow_rel", true, "legacy_exposed");
  }
  if (phase === "P5_MIXED") {
...
function nextTransferState(state: TransferControllerState, input: WapUserState): TransferControllerState {
  ...
  if (state.phase === "base_fluency") {
    ...
    if ((startEvidence?.validTrials || 0) >= 80 && !state.transferEvents.some((item) => item.eventType === "diagnostic_probe")) {
      const next = {
        ...state,
  ...
  if (state.phase === "transition_probe") {
    return { ...state, phase: "expected_dip", activeTargetWrapper: state.activeTargetWrapper || path.carrierTargetWrapper };
  }

  if (state.phase === "expected_dip") {
    return { ...state, phase: "recovering", activeTargetWrapper: state.activeTargetWrapper || path.carrierTargetWrapper, mixRatio: null, activeMix: null };
  }
  ...
  if (state.phase === "recovering") {
    ...
    if (!readyToMix(source, targetEvidence)) {
    ...
    if (target === path.carrierTargetWrapper) {
      return appendEvent(
        ...
        event("recovery", path.startWrapper, path.carrierTargetWrapper, input.sessionNumber, "recovering", null, false, {
          recoveryRatio: recoveryRatio(source, targetEvidence),
        }),
      );
    }
      ...
      return appendEvent(
        {
          ...
          phase: "mix_80_20",
          activeBaseWrapper: path.startWrapper,
          activeTargetWrapper: path.frameTargetWrapper,
          mixRatio: 0.2,
          activeMix: mixFor(path.startWrapper, path.frameTargetWrapper, 0.2),
        },
    ...
    return appendEvent(
      {
        ...
        activeTargetWrapper: path.carrierTargetWrapper,
        mixRatio: 0.2,
        activeMix: mixFor(path.startWrapper, path.carrierTargetWrapper, 0.2),
      },
  ...
  if (state.phase === "random_mix") {
    ...
    return {
      ...
      mixRatio: null,
      activeMix: null,
      legacyStatus: state.legacyStatus === "none" ? "rebaseline_required" : state.legacyStatus,
    };
  }
    ...
    return appendEvent(
      {
        ...
        delayedRechecks: state.delayedRechecks.length
          ...
          : [{
            id: `delayed-${input.sessionNumber}`,
            dueAfterSession: input.sessionNumber + TRANSFER_DEFAULTS.delayedRecheckMinSessionsAfterMix,
